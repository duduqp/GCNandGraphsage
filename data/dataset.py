...
def load_data(opt):
    ...
    idx_features_labels = np.genfromtxt("./data/{}.content" .format(opt.network),
                                        dtype=np.dtype(str))
    ...
    edges_unordered = np.genfromtxt("./data/{}.cites".format(opt.network),
                                    dtype=np.int32)
    edges = np.array(list(map(idx_map.get, edges_unordered.flatten())),
                     dtype=np.int32).reshape(edges_unordered.shape) # 编码到编号的转换
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        shape=(labels.shape[0], labels.shape[0]),
                        dtype=np.float32) #构建邻接矩阵
    ...
    idx_val = list()
    idx_test = list()
    # print("labels_map(dict) size:")
    # print(len(labels_map))
    for ele in labels_map:#ele 表示七类别中的一类
        idx_train.extend(labels_map[ele][0:int(opt.train_rate * labels.shape[0])])
        idx_val.extend(labels_map[ele][int(opt.train_rate * labels.shape[0]):
                                       int((opt.train_rate + opt.val_rate) * labels.shape[0])])
